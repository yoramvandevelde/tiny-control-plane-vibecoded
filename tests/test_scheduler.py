import os
import time
from controller.store import init_db, register_node, update_state
...
def test_pick_node_excludes_stale_node(tmp_path):
    ...
    register_node("node1", "http://localhost:9000")
    # update state but backdate last_seen to simulate stale node
...
def test_pick_node_selects_least_loaded(tmp_path):
    _setup(tmp_path)
